...
**Step 3 — `api/v3/editor.py` (BLOCKER-1 fix — D-17):** Add `/editor/apply` endpoint accepting `{op_type, payload, branch_id}`. The handler MUST NOT mutate geometry out-of-band. It:
...
<task type="auto" tdd="true">
  ...
  <action>
**Step 1 — `useEditorStore.ts`:** Add 3 undoable actions that build optimistic local geometry FIRST, then POST /editor/apply for persistence (BLOCKER-1 fix — the API does not return geometry):
